...
typedef struct Medicine {
    ...
    struct Medicine* prev;
} medicine;
int mCount = 0;

typedef struct uS_Tag { // for unique suppliers
    long int supplier_id;
    int noOfUniqueMedicine;
    float totalTurnOver;

    struct uS_Tag* nextUniqueSupplier;
    struct uS_Tag* prevUniqueSupplier;
} uniqueSupplier;

uniqueSupplier* uniqueSuppliers = NULL;
int uniqueSupplierCount = 0;
int flag = 1;

void totalNumberOfSuppliers(int supplierId, int quantity, int price_per_unit) {
    uniqueSupplier* temp = uniqueSuppliers;
    ...
    if (!uniqueSuppliers) {
        uniqueSuppliers = newSupplier;
        return;
    }

    temp = uniqueSuppliers;
...
void saveData(medicine* medicationHead) {
    FILE* file = fopen("pharmacy.txt", "a");
    if (!file) {
    ...
    while (temp) {
        ...
        while (tempSupplier) {
            ...
            tempSupplier = tempSupplier->next;
        }
    ...
    uniqueSupplier* tempUnique = uniqueSuppliers;
    while (tempUnique) {
...
void loadData(medicine** medicationHead, medicine** medicationTail) {
    ...
    while (!feof(file)) {
        ...
        while (1) {
            ...
            if (lastSupplier) {
                lastSupplier->next = newSupplier;
                newSupplier->prev = lastSupplier;
            } else {
        ...
        newMedicine->prev = *medicationTail;
        if (*medicationTail) {
            (*medicationTail)->next = newMedicine;
        } else {
...
void displayMedication(medicine* medicine) {
    ...
    while (medicine->suppliers) {
        ...
        medicine->suppliers = medicine->suppliers->next;
        j++;
    }
        ...
        while (temp && temp->nextUniqueSupplier) {
            ...
            if (temp->totalTurnOver < nextNode->totalTurnOver) {
                ...
                if (prev) {
                    ...
                    uniqueSuppliers = nextNode;  // Update head if first node is swapped
                }
        ...
        if(temp->medication_id == id){
            ...
            if ( ch == 1 ){
                ...
                while(temp->suppliers && flag == 1){
                    ...
                    temp->suppliers = temp->suppliers->next;
                }
            }
        }
        else temp = temp->next;
...
void add_New_Medication(medicine** medicineHead, medicine** medicineTail){
    ...
    if((*medicineHead) == NULL){
        ...
        *medicineTail = *medicineHead;
        ...
        (*medicineTail)->next = (medicine*)malloc(sizeof(medicine));
        (*medicineTail)->next->prev = *medicineTail;
        initMedicine(&(*medicineTail)->next);
        *medicineTail = (*medicineTail)->next;
    }
}
...
int main() {
    ...
    loadData(&Head, &Tail); // Load data at the start
    ...
    while (1) {
        ...
        printf("7. Sort Medication by Expiration Date\n");
        printf("8. Sales Tracking\n");
        printf("9. Supplier Management\n");
        printf("10. Find all-rounder suppliers\n");
        printf("11. Find suppliers with largest turn-over\n");
        printf("12. Display all medicines\n");
        printf("13. Exit\n");
        printf("\n");
        printf("-------------------------------------------------------------------------------------\n");
        ...
        switch (ch) {
            case 1:
                add_New_Medication(&Head, &Tail);
                saveData(Head);
                break;
            case 2:
                update_Medication_Details(&Head);
                saveData(Head);
                break;
            case 3:
                delete_Medication(&Head);
                saveData(Head);
                break;
            case 4:
                search_Medication(Head);
                break;
            case 5:
                stockAlert(Head);
                break;
            case 6:
                check_Expiration_Dates(Head);
                break;
            case 7:
                sort_Medication_by_Expiration_Dates(&Head);
                saveData(Head);
                break;
            case 8:
                salesTracking(Head);
            ...
            case 10:
                ...
                break;
            case 11:
                find_suppliers_with_largest_turn_over();
                break;
            case 12:
                displayAllMedications(Head);
                break;
            case 13:
                printf("Exiting...\n");
                saveData(Head); // Save data before exiting
                return 0;
            default:
                printf("Invalid option!\n");
                break;
